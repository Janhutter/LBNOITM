...
# error margin for match and recall which is standard deviation of the mean
def match_score_error_margin(predictions, references):
    return np.std([max([match_single(prediction, gt) for gt in ground_truths]) for ground_truths, prediction in zip(references, predictions)] / np.sqrt(len(predictions)))

def recall_error_margin(predictions, references):
    return np.std([max([rec for _, _, rec in [f1_single(prediction, gt) for gt in ground_truths]]) for ground_truths, prediction in zip(references, predictions)] / np.sqrt(len(predictions)))
...
class RAGMetrics:
    ...
    def compute(predictions, references, questions=None):
        ...
        return {    "M": match_score(predictions, references),
                    ...
                    "Rouge-2": rouge2,
                    "Rouge-L": rougel,
                    "M error": match_score_error_margin(predictions, references),
                    "Recall error": recall_error_margin(predictions, references)
                }
