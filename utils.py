...
def prepare_dataset_from_ids(dataset, q_ids, d_ids, multi_doc=False, query_embeds_path=None, doc_embeds_path=None, sample_n=None):
    ...
    datasets_dict = datasets.Dataset.from_dict(dataset_dict)

    # sample questions randomly
    if sample_n is not None:
        datasets_dict = datasets_dict.shuffle(seed=42).select(range(sample_n))
...
def eval_retrieval_kilt(experiment_folder, qrels_folder, dataset_name, split, query_ids, doc_ids, scores, top_k=5, reranking=False, debug=False, write_trec=True):
    ...
    if not all(sublist for sublist in doc_ids):
        return
...
def get_ranking_filename(runs_folder, query_dataset, doc_dataset, retriever_name, dataset_split, retrieve_top_k, trec_file=None):
    if trec_file is not None:
        return f'{runs_folder}/run.{trec_file}.trec'
